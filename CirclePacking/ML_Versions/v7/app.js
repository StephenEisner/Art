    ...
    for (let i = 0; i < k; i++) {
      ...
      for (let c = 0; c < components.length; c++) {
        ...
        if (islandArea < 20) {
          ...
          console.log(`  Island ${c + 1}: ${islandArea}px → Pixel fill (${shapes.length} pixels)`);
          ...
          console.log(`  Island ${c + 1}: ${islandArea}px → Micro-pack (${shapes.length} shapes)`);
          
        } else {
          // Strategy 3: Full multi-pass packing for larger islands
          setProgress(`Packing cluster ${i + 1}/${k}, island ${c + 1}/${components.length}`);
          
          shapes = await Packing.packCircles(island, color, img.width, img.height, {
            minRadius: minCircleRadius,
            maxRadius: maxCircleRadius,
            maxAttempts: Math.floor(maxAttempts * (islandArea / regions[i].length)), // Proportional attempts
            shapesPerBatch: 100,
            shapeType: shapeType,
            colorMode: colorMode,
          ...
          console.log(`  Island ${c + 1}: ${islandArea}px → Full pack (${shapes.length} shapes)`);
        }
