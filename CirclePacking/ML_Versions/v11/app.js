    ...
    for (let i = 0; i < k; i++) {
      ...
      for (let c = 0; c < components.length; c++) {
        ...
        if (islandArea < 20) {
          ...
          const finalAttempts = useMultiPassForIsland ? Math.floor(allocatedAttempts * 1.5) : allocatedAttempts;
        ...
        if (c < 5 || c === components.length - 1 || components.length <= 10) {
          console.log(`  Island ${c + 1}/${components.length}: ${islandArea}px → ${strategy}${multiPassUsed} → ${allocatedAttempts} attempts → ${shapes.length} shapes → ${islandCoverage}% coverage`);
        } else if (c === 5) {
