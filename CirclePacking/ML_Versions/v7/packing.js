...
const Packing = {
  ...
  async microPack(region, defaultColor, width, height, options = {}) {
    ...
    const shapes = [];
    const minRadius = 0.5;
    const maxRadius = Math.min(5, options.minRadius || 3);
    const maxAttempts = Math.min(region.length * 20, 2000);
    ...
    let consecutiveFailures = 0;
    const maxFailures = Math.floor(maxAttempts / 10);
    ...
    for (let attempt = 0; attempt < maxAttempts && consecutiveFailures < maxFailures; attempt++) {
      ...
      // Try to fit largest possible micro circle
      let bestRadius = 0;
      for (let r = maxRadius; r >= minRadius; r -= 0.5) {
        const testShape = { type: 'circle', x, y, r, color: defaultColor };
