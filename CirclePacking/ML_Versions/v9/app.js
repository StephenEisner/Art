    ...
    for (let i = 0; i < k; i++) {
      ...
      for (let c = 0; c < components.length; c++) {
        ...
        if (islandArea < 20) {
          ...
          // Calculate base attempts (increased from 1.5x to 2x multiplier)
          const baseAttempts = Math.max(
            1500, // Increased minimum from 1000
            Math.floor(maxAttempts * (islandArea / clusterPixelsTotal) * 2)
          );
          
          // ADAPTIVE MULTI-PASS: Only use for large islands where it helps
          // Small islands (<1000px) work better with single-pass + more attempts
          const useMultiPassForIsland = enableMultiPass && islandArea >= 1000;
          
          // Give 50% more attempts when using multi-pass (to compensate for pass division)
